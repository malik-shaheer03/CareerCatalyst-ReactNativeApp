...
const ResumePreview: React.FC<ResumePreviewProps> = ({
  ...
  const fontSizes = {
    small: { base: 12, heading: 18, subheading: 14, small: 10 },
    medium: { base: 14, heading: 22, subheading: 16, small: 12 },
    large: { base: 16, heading: 26, subheading: 18, small: 14 }
  };

  const currentTheme = themes[selectedTheme as keyof typeof themes];
  const currentFontSize = fontSizes[fontSize as keyof typeof fontSizes];
      ...
      <View style={styles.personalHeader}>
        ...
        {resumeData.personal?.avatar && (
          <View style={styles.avatarContainer}>
            <View style={[styles.avatar, { borderColor: currentTheme.primaryColor }]}>
              <Text style={[styles.avatarText, { color: currentTheme.primaryColor }]}>
                {(resumeData.personal.firstName?.[0] || '') + (resumeData.personal.lastName?.[0] || '')}
              </Text>
            </View>
          </View>
        )}
      ...
      <View style={[styles.header, { borderBottomColor: currentTheme.borderColor }]}>
        ...
        <View style={styles.headerControls}>
          ...
          <View style={styles.controlGroup}>
            <Text style={[styles.controlLabel, { color: currentTheme.lightTextColor }]}>Theme:</Text>
            <View style={styles.themeButtons}>
              {Object.keys(themes).map((theme) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.themeButton,
                    selectedTheme === theme && { backgroundColor: currentTheme.primaryColor }
                  ]}
                  onPress={() => setSelectedTheme(theme)}
                >
                  <Text style={[
                    styles.themeButtonText,
                    { color: selectedTheme === theme ? '#fff' : currentTheme.textColor }
                  ]}>
          ...
          {/* Font size selector */}
          <View style={styles.controlGroup}>
            <Text style={[styles.controlLabel, { color: currentTheme.lightTextColor }]}>Size:</Text>
              ...
              {Object.keys(fontSizes).map((size) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    fontSize === size && { backgroundColor: currentTheme.primaryColor }
                  ]}
                  onPress={() => setFontSize(size)}
                >
                  <Text style={[
                    styles.fontSizeButtonText,
                    { color: fontSize === size ? '#fff' : currentTheme.textColor }
                  ]}>
          ...
          <View style={styles.actionButtons}>
            <TouchableOpacity
              style={[styles.actionButton, { backgroundColor: currentTheme.secondaryColor }]}
              onPress={handleShare}
            >
              <Icon name="share" size={16} color={currentTheme.primaryColor} />
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.actionButton, { backgroundColor: currentTheme.secondaryColor }]}
              onPress={handleExport}
            >
              <Icon name="download" size={16} color={currentTheme.primaryColor} />
            </TouchableOpacity>
          </View>
        </View>
      </View>
